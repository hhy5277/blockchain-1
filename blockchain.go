...
func wsHandleP2P(ws *websocket.Conn) {
	...
	for {
		...
		case queryLatest:
			...
			log.Printf("responseLatestMsg: %s", bs)
			ws.Write(bs)
...
func replaceChain(bc []*Block) {
	if isValidChain(bc) && len(bc) > len(blockchain) {
		log.Println("Recevied blockchain is valid. Replacing current blockchain with recevied blockchain.")
		blockchain = bc
		broadcast(responseLatestMsg())
	} else {
		log.Println("Recevied blockchain invalid.")
	}
}
...
func handleBlockchainResponse(msg []byte) {
	...
	if latestBlockReceived.Index > latestBlockHeld.Index {
		log.Printf("blockchain possibly behind. We got: %d Peer got: %d",
			latestBlockHeld.Index, latestBlockReceived.Index)
